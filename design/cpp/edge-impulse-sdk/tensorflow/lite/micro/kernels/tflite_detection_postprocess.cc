...
void* Init(TfLiteContext* context, const char* buffer, size_t length) {
  ...
  printf("postprocess init 5\n");

  printf("use_regular_non_max_suppression = %d\n", op_data->use_regular_non_max_suppression);
  ...
  printf("op_data->max_detections %d\n", op_data->max_detections);
  printf("op_data->max_classes_per_detection %d\n", op_data->max_classes_per_detection);
  printf("op_data->detections_per_class %d\n", op_data->detections_per_class);

  printf("postprocess init 7\n");

  return op_data;
}

void Free(TfLiteContext* context, void* buffer) {
  printf("postprocess free\n");
}

TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  printf("postprocess prepare\n");

  auto* op_data = static_cast<OpData*>(node->user_data);

  printf("postprocess prepare 2\n");

  // Inputs: box_encodings, scores, anchors
  TF_LITE_ENSURE_EQ(context, NumInputs(node), 3);
  printf("postprocess prepare 3\n");
  const TfLiteTensor* input_box_encodings =
      GetInput(context, node, kInputTensorBoxEncodings);
  printf("postprocess prepare 4\n");
  const TfLiteTensor* input_class_predictions =
      GetInput(context, node, kInputTensorClassPredictions);
  printf("postprocess prepare 5\n");
  const TfLiteTensor* input_anchors =
      GetInput(context, node, kInputTensorAnchors);
  printf("postprocess prepare 6\n");
  // printf("%d\n", input_box_encodings->dims->size);
  ...
  printf("%p %p %p\n",
    ...
    (input_anchors));
  printf("%p %p %p\n",
    (input_box_encodings->dims),
    (input_class_predictions->dims),
    (input_anchors->dims));
  TF_LITE_ENSURE_EQ(context, NumDimensions(input_box_encodings), 3);
  TF_LITE_ENSURE_EQ(context, NumDimensions(input_class_predictions), 3);
  TF_LITE_ENSURE_EQ(context, NumDimensions(input_anchors), 2);
  printf("postprocess prepare 7\n");

  TF_LITE_ENSURE_EQ(context, NumOutputs(node), 4);
  const int num_boxes = input_box_encodings->dims->data[1];
  const int num_classes = op_data->num_classes;
  printf("postprocess prepare 8\n");
  ...
  printf("postprocess prepare 12\n");

  // Outputs: detection_boxes, detection_scores, detection_classes,
  // num_detections
  TF_LITE_ENSURE_EQ(context, NumOutputs(node), 4);
  printf("postprocess prepare 13\n");
